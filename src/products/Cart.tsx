...
interface CartProps {
  showToastMessage: (message: string, isSuccess: boolean) => void;
}

const Cart: React.FC<CartProps> = ({ showToastMessage }) => {
  ...
  const subtotal = cartItems.reduce(
    (total, item) => total + (item.totalPrice || (item.productPrice * item.productQuantity)),
    0
  );
  ...
  const handleQuantityChange = async (itemId: string, newQuantity: number) => {
    try {
      ...
      if (!currentUser) {
        showToastMessage("Please log in to update cart", false);
        return;
      }

      const cartItemRef = doc(db, "customers", currentUser.uid, "cart", itemId);
      const cartItemDoc = await getDoc(cartItemRef);
      
      if (!cartItemDoc.exists()) {
        showToastMessage("Item not found in cart", false);
        return;
      }

      const cartItem = cartItemDoc.data();
      const basePrice = cartItem.productPrice || cartItem.productSize?.price || 0;
      ...
      showToastMessage("Cart updated successfully", true);
    } catch (error) {
      console.error("Error updating quantity:", error);
      showToastMessage("Failed to update cart", false);
    }
  };
